...
from novel_writer.models import Chapter, Scene
from novel_writer.modules.anti_slop.scorer import score_document
from novel_writer.modules.bible.service import list_characters
from novel_writer.modules.outline.service import list_outlines
...
def _render_generate(novel_id: str) -> None:
    ...
    if st.button("🚀 生成场景", type="primary", use_container_width=True):
        from novel_writer.modules.generation.service import generate_scene
            ...
            with get_session() as session:
                ...
                if ch_obj and ch_obj.summary:
                    ...
                    if st.button(f"生成摘要", key=f"summary_{ch.id}"):
                        from novel_writer.modules.generation.service import generate_chapter_summary
                        loop = _get_event_loop()
